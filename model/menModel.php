<?php
    ...
    class menModel extends Connect
    {
        ...
        protected function getProductAllMen($from, $limit)
        {
            $sql = "SELECT tbl_products.id, tbl_category_products.name_cate, tbl_brand.name_brand, tbl_products.name, tbl_products.main_image, tbl_products.price, tbl_products.description, tbl_products.sale, tbl_products.views, tbl_products.status, tbl_products.created_at, tbl_products.updated_at FROM tbl_products, tbl_category_products, tbl_brand WHERE tbl_products.cate_pro_id = tbl_category_products.id AND tbl_products.brand_id = tbl_brand.id AND tbl_category_products.gender_product = 2 ORDER BY views DESC LIMIT  $from, $limit";
        ...
        protected function countAllProduct_m(){
            $sql = "SELECT COUNT(tbl_products.id) AS 'countAll' FROM tbl_products, tbl_category_products, tbl_brand WHERE tbl_products.cate_pro_id = tbl_category_products.id AND tbl_products.brand_id = tbl_brand.id AND tbl_category_products.gender_product = 2";
            $result = $this->pdo->prepare($sql);
            $result->execute();
